...
public class AndroidTreeView {
    ...
    private void collapseNode(TreeNode node, final boolean includeSubnodes) {
        ...
        TreeNode.BaseNodeViewHolder
                nodeViewHolder = getViewHolderForNode(node);
        ...
        nodeViewHolder.toggle(false, node);
        if (includeSubnodes) {
            for (TreeNode n : node.getChildren()) {
    ...
    private void expandNode(final TreeNode node, boolean includeSubnodes) {
        ...
        parentViewHolder.toggle(true, node);
    ...
    private void addNode(ViewGroup container, final TreeNode n) {
        ...
        //AlphaAnimation mAlphaAnimation = new AlphaAnimation(0.5f, 1.0f);
        //mAlphaAnimation.setDuration(240);
        //mAlphaAnimation.setFillAfter(true);

        TranslateAnimation mTranslateAnimation = new
                TranslateAnimation(Animation.RELATIVE_TO_PARENT, 0,
                Animation.RELATIVE_TO_PARENT, 0, Animation.ABSOLUTE,
                -20, Animation.ABSOLUTE, 0);
        mTranslateAnimation.setDuration(250);
        ...
        nodeView.startAnimation(mAnimationSet);

        if (mSelectionModeEnabled) {
            viewHolder.toggleSelectionMode(mSelectionModeEnabled);
        }
        if (n.getChildren().size() == 0) {
            if (n.getViewHolder() instanceof TreeAdapter)
                ((TreeAdapter) n.getViewHolder()).getImg().setVisibility(View.GONE);
        }
