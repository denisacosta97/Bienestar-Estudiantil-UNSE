...
public class ImpresionesDiaActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    Impresion mImpresion;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_impresiones_dia);

        if (getIntent().getParcelableExtra(Utils.DATA_TURNO) != null)
            mImpresion = getIntent().getParcelableExtra(Utils.DATA_TURNO);
    ...
    private void loadInfo() {
        ...
        String URL = "";
        if (mImpresion == null)
             URL = String.format("%s?idU=%s&key=%s", Utils.URL_IMPRESIONES_HOY, id, key);
        else URL = String.format("%s?idU=%s&key=%s&di=%s&me=%s&an=%s", Utils.URL_IMPRESIONES_DIA, id, key,
                mImpresion.getDia(), mImpresion.getMes(), mImpresion.getAnio());
