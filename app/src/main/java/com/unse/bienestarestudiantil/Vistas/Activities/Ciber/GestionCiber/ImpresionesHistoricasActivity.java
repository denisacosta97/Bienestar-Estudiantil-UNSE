...
public class ImpresionesHistoricasActivity extends AppCompatActivity implements View.OnClickListener {

    DialogoProcesamiento dialog;
    ArrayList<Impresion> mImpresion;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_impresiones_historicas);
    ...
    private void setToolbar() {
        ...
        ((TextView) findViewById(R.id.txtTitulo)).setText("Historial de Impresiones");
    }
    ...
    private void loadInfo() {
        ...
        String URL = String.format("%s?idU=%s&key=%s", Utils.URL_IMPRESIONES_HIST, id, key);
    ...
    private void loadInfo(JSONObject jsonObject) {
        try {
            if (jsonObject.has("mensaje")) {
                ...
                mImpresion = new ArrayList<>();

                for (int i = 0; i < jsonArray.length(); i++) {

                    JSONObject o = jsonArray.getJSONObject(i);

                    Impresion imp = Impresion.mapper(o, Impresion.MEDIUM);
                    mImpresion.add(imp);
                }

                mItems = new ArrayList<>();
                for (Impresion m : mImpresion) {
                    ItemDato itemDato = new ItemDato();
                    itemDato.setImpre(m);
                    itemDato.setTipo(ItemDato.TIPO_IMPRE);
                    mItems.add(itemDato);
                }
                ...
                adapter = new FechasAdapter(getApplicationContext(), mListOficial, FechasAdapter.TIPO_IMPRE);
        ...
        itemClickSupport.setOnItemClickListener(new ItemClickSupport.OnItemClickListener() {
            ...
            public void onItemClick(RecyclerView parent, View view, int position, long id) {
                if (mListOficial.get(position) instanceof ItemDato) {
                    Intent i = new Intent(getApplicationContext(), ImpresionesDiaActivity.class);
                    Impresion imp = ((ItemDato) mListOficial.get(position)).getImpre();
                    i.putExtra(Utils.DATA_TURNO, imp);
                    startActivity(i);
                }
            }
        });
    ...
    private HashMap<String, List<ItemBase>> filtrarPorMes(List<ItemBase> list) {
        ...
        for (ItemBase dato : list) {
            ...
            if (itemDatoKey.getTipoDato() == ItemDato.TIPO_IMPRE) {

                String mes = String.valueOf(itemDatoKey.getImpre().getMes());
                ...
                if (!groupedHashMap.containsKey(mes)) {
                    ...
                    for (ItemBase item : list) {
                        ...
                        if (itemDato.getImpre().getMes().equals(itemDatoKey.getImpre().getMes())) {
                            if (groupedHashMap.containsKey(mes)) {
